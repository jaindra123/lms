...
class qformat_default {
    ...
    public function can_import_file($file) {
        $expected = $this->mime_type();
        $actual = $file->get_mimetype();
        if ($actual === $expected) {
            return true;
        }
        // Plain-text question formats (.txt, .gift, .csv) are often detected as text/csv on upload.
        if ($expected === 'text/plain') {
            ...
            if (in_array($actual, $aliases, true)) {
                return true;
            }
        }
        return false;
