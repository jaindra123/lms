...
/**
 * Test the iiidem2navbar file
 *
 * @package    theme_iiidem2
 * @covers     \theme_iiidem2\iiidem2navbar
 * @copyright  2021 Peter Dias
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class iiidem2navbar_test extends \advanced_testcase {
    /**
    ...
    public function test_remove_no_link_items(array $setup, bool $removesectionnodes, array $expected): void {
        ...
        $iiidem2navbar = $this->getMockBuilder(iiidem2navbar::class)
            ->disableOriginalConstructor()
            ->onlyMethods([])
            ->getMock();

        $rc = new \ReflectionClass(iiidem2navbar::class);
        $rcp = $rc->getProperty('items');
        $rcp->setValue($iiidem2navbar, $page->navbar->get_items());

        // Make the call to the function.
        $rcm = $rc->getMethod('remove_no_link_items');
        $rcm->invoke($iiidem2navbar, $removesectionnodes);

        // Get the value for the class variable that the function modifies.
        $values = $rcp->getValue($iiidem2navbar);
        $actual = [];
    ...
    public function test_remove_items_that_exist_in_navigation(string $navmenu, array $navmenunodes, array $navbarnodes,
        ...
        $rcm->invoke($iiidem2navbar, $navigationmenu);
