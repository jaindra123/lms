...
/**
 * Creates a navbar for iiidem2 that allows easy control of the navbar items.
 *
 * @package    theme_iiidem2
 * @copyright  2021 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class iiidem2navbar implements \renderable {
    ...
    public function __construct(\moodle_page $page) {
        ...
        $this->prepare_nodes_for_iiidem2();
    }

    /**
     * Prepares the navigation nodes for use with iiidem2.
     */
    protected function prepare_nodes_for_iiidem2(): void {
        global $PAGE;
    ...
    /**
     * Get all the iiidem2navbaritem elements.
     *
     * @return iiidem2navbaritem[] iiidem2 navbar items.
     */
    ...
    /**
     * Removes all navigation items out of this iiidem2 navbar
     */
    ...
    /**
     * Remove a iiidem2navbaritem from the iiidem2 navbar.
     *
     * @param  string|int $itemkey An identifier for the iiidem2navbaritem
     * @param  int|null $itemtype An additional type identifier for the iiidem2navbaritem (optional)
     */
    ...
    /**
     * Removes the action from the last item of the iiidem2navbaritem.
     */
    ...
    protected function get_node_text_and_action(navigation_node $node): array {
        ...
        if ($node->has_action()) {
            if ($node->action instanceof moodle_url) {
                ...
                $action = is_object($node->action) && method_exists($node->action, 'out')
                    ? $node->action->out()
                    : (string)$node->action;
            } else if ($node->action instanceof action_link) {
               // $action = $node->action->url->out();
                $action = (is_object($node->action) &&
                       isset($node->action->url) &&
                       method_exists($node->action->url, 'out'))
                ? $node->action->url->out()
                : '';
            } else {
                $action = $node->action;
