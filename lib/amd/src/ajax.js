 ...
 */
    ...
    var requestSuccess = function(responses) {
        ...
        if (exception !== null) {
            ...
            if (exception.errorcode === "servicerequireslogin" && !nosessionupdate) {
                window.location = URL.relativeUrl("/login");
            } else {
    ...
    return /** @alias module:core/ajax */ {
        ...
        call: function(requests, async, loginrequired, nosessionupdate, timeout, cachekey) {
            ...
            // CDAC #17: never put sesskey in the query string (Referer / proxy / history).
            // CSRF is sent as X-Moodle-Sesskey; theme maps the header when query sesskey is absent.
            if (!loginrequired) {
            ...
            if (config.sesskey) {
                ...
                settings.headers['X-Moodle-Sesskey'] = config.sesskey;
            }
